...
export function GameStartForm({ suggestions }: { suggestions: string[] }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-[28px] border border-brand-border bg-[#20110c] p-5 text-white shadow-elevated sm:p-8">
      ...
      <div className="relative grid gap-7 lg:grid-cols-[1.15fr_.85fr] lg:items-center"><div><div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-bold tracking-wide text-[#f8c5a9]"><Sparkles size={14} /> MODO MISSÃO INFINITA</div><h2 className="mt-4 font-display text-3xl font-extrabold leading-tight sm:text-4xl">Aprender virou uma jornada.</h2><p className="mt-3 max-w-lg text-sm leading-relaxed text-white/70">Escolha uma matéria e avance sem limite. A missão começa acessível, evolui para avançada e alcança níveis extremos conforme você progride.</p><div className="mt-5 flex flex-wrap gap-2 text-xs font-bold"><span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-2"><Heart size={14} className="fill-[#ff7b6d] text-[#ff7b6d]" /> 3 energias</span><span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-2"><Zap size={14} className="text-[#ffd166]" /> combos dão mais XP</span><span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-2"><Trophy size={14} className="text-[#ffd166]" /> níveis extremos</span></div></div><div className="relative mx-auto flex w-full max-w-[280px] items-center justify-center"><div className="absolute size-52 rounded-full border border-brand/40" /><div className="absolute size-40 rounded-full border border-dashed border-[#f8c5a9]/40" /><span className="relative flex size-28 items-center justify-center rounded-[30px] border border-white/20 bg-gradient-to-br from-brand to-[#9d3f1f] shadow-[0_0_45px_rgba(217,119,87,.55)]"><Crown size={46} /></span><span className="absolute right-1 top-4 rounded-xl bg-[#ffd166] px-2 py-1 text-xs font-black text-[#3c2500]">+100 XP</span><span className="absolute bottom-3 left-0 rounded-xl bg-white/15 px-2 py-1 text-xs font-bold">Sem fim</span></div></div>
