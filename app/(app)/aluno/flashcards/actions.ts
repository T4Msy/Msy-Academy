...
export async function createDeck(title: string): Promise<string> {
  ...
  if (!clean) throw new Error("Dê um título ao conjunto de cartões.");

  const { supabase, user } = await requireUser();
  const { data: profile } = await supabase.from("profiles").select("tenant_id").eq("id", user.id).single();
  if (!profile) throw new Error("Não encontramos seu perfil. Atualize a página e tente novamente.");
  ...
  const { data, error } = await supabase
    ...
    .select("id")
    .single();
  if (error || !data) throw new Error("Não conseguimos criar o conjunto de cartões. Tente novamente.");
...
export async function deleteDeck(deckId: string): Promise<void> {
  ...
  if (error) throw new Error("Não conseguimos excluir o conjunto de cartões. Tente novamente.");
