...
type EnrollmentRow = { class_id: string; created_at: string; classes: { id: string; name: string; invite_code: string; created_at: string; subject_id: string | null } | null };
...
function maskInviteCode(code: string): string { const visiblePrefix = code.slice(0, 3); return `${visiblePrefix}${"•".repeat(Math.max(3, code.length - visiblePrefix.length))}`; }

export default async function AlunoTurmasPage() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  const { data: enrollments } = await supabase.from("enrollments").select("class_id, created_at, classes(id, name, invite_code, created_at, subject_id)").eq("student_id", user!.id).eq("status", "ACTIVE").order("created_at", { ascending: false });
