    ...
    <div className="flex flex-col gap-2.5 p-0">
      <div className="grid grid-cols-1 gap-3.5 min-[861px]:grid-cols-2">
        <div className="flex flex-col gap-1.5">
          ...
          <input data-testid={`grade-score-${questionId}`} id={`score-${questionId}`} className="w-full appearance-none rounded-sm border border-border bg-[rgba(var(--overlay-rgb),0.04)] px-3 py-2.5 text-md text-foreground outline-none transition-colors focus:border-brand-border focus:ring-[3px] focus:ring-brand-glow" type="number" min={0} max={1} step={0.05} value={score} onChange={(e) => setScore(e.target.value)} />
        </div>
        <div className="flex flex-col gap-1.5 sm:col-span-2">
          <label className="block text-sm font-semibold text-foreground" htmlFor={`feedback-${questionId}`}>Feedback</label>
          <textarea data-testid={`grade-feedback-${questionId}`} id={`feedback-${questionId}`} className="w-full appearance-none rounded-sm border border-border bg-[rgba(var(--overlay-rgb),0.04)] px-3 py-2.5 text-md text-foreground outline-none transition-colors focus:border-brand-border focus:ring-[3px] focus:ring-brand-glow" value={feedback} onChange={(e) => setFeedback(e.target.value)} />
        </div>
      </div>
