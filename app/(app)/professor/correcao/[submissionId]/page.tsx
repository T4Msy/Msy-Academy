...
export default async function CorrecaoDetailPage({ params, searchParams }: { params: Promise<{ submissionId: string }>; searchParams: Promise<{ turma?: string }> }) {
  ...
  return <>
    ...
    {!completed && !objectiveNeedsKey && <CorrecaoReview submissionId={submissionId} objectiveScoreSum={objectiveScoreSum} discursivas={discursivas} returnHref={turma ? `/professor/correcao/turma/${turma}?view=history` : "/professor/correcao"} />}
  </>;
}
