...
export default async function CorrecaoDetailPage({ params }: { params: Promise<{ submissionId: string }> }) {
  ...
  return <>
    ...
    {!completed && discursivas.length === 0 && <div className="mt-3.5 rounded-md border border-brand-border bg-brand-dim px-4.5 py-3.5 text-[13.5px] leading-normal text-brand-text">Sem questões discursivas para corrigir neste envio.</div>}
    {!completed && discursivas.length > 0 && <CorrecaoReview submissionId={submissionId} objectiveScoreSum={objectiveScoreSum} discursivas={discursivas} />}
  </>;
}
