...
export default async function CorrecaoPage() {
  ...
  const pendingTotal = [...totals.values()].reduce((total, item) => total + item.pending, 0);
  const gradedTotal = [...totals.values()].reduce((total, item) => total + item.graded, 0);
  const classesWithPending = [...totals.values()].filter((item) => item.pending > 0).length;
  return <>
    <div className="mb-6"><h1 className="font-display text-3xl font-extrabold tracking-[-0.6px] text-foreground">Correções</h1><p className="mt-1 text-[13.5px] text-muted-foreground">Acompanhe e corrija as entregas dos seus alunos.</p></div>
    <div className="mb-6 grid gap-3 sm:grid-cols-3"><Summary label="Aguardando correção" value={pendingTotal} emphasis /><Summary label="Corrigidas" value={gradedTotal} /><Summary label="Turmas com pendências" value={classesWithPending} /></div>
...
function Summary({ label, value, emphasis = false }: { label: string; value: number; emphasis?: boolean }) { return <div className="rounded-lg border border-border bg-card p-4"><p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{label}</p><p className={`mt-2 font-display text-2xl font-bold ${emphasis ? "text-brand-text" : "text-foreground"}`}>{value}</p></div>; }
