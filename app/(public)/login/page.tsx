  ...
  return (
    <div className="grid min-h-screen place-items-center px-5 py-10">
      <div className="w-full max-w-[420px] rounded-lg border border-border bg-card p-7 pt-8 shadow-elevated">
        ...
        {message && <div data-testid="login-message" className="mt-3.5 rounded-md border border-brand-border bg-brand-dim px-4.5 py-3.5 text-[13.5px] leading-normal text-brand-text">{message}</div>}
        {error && <div data-testid="login-error" className="mt-3.5 rounded-md border border-danger-border bg-danger-dim px-4.5 py-3.5 text-[13.5px] leading-normal text-danger-text">{error}</div>}

        <GoogleSignInButton redirectTo="/" />
        <div className="my-4 flex items-center gap-3 text-xs text-subtle"><span>ou</span></div>

        <form className="mt-2 flex flex-col gap-3.5" action={login} data-testid="login-form">
          ...
          <TurnstileWidget />
          <button data-testid="login-submit" type="submit" className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-sm text-md font-semibold transition-all outline-none focus-visible:ring-[3px] focus-visible:ring-brand-glow active:translate-y-px disabled:pointer-events-none disabled:opacity-50 bg-primary font-bold text-primary-foreground shadow-[0_4px_14px_rgba(217,119,87,0.16)] hover:-translate-y-px hover:opacity-90 px-4 py-2.5 w-full">
            Entrar
          </button>
        </form>
