...
export function StudentMissionsCard({ initialCompletedIds }: { initialCompletedIds: StudentMissionId[] }) {
  ...
  return (
    <Card className="mb-6 gap-0 overflow-hidden border-brand-border bg-card py-0 shadow-elevated">
      <CardHeader className="gap-0 border-b border-border px-5.5 py-5">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2.5">
              <CardTitle className="font-display text-lg font-bold text-foreground">Comece por aqui</CardTitle>
        ...
        <div className="mt-4 h-2 overflow-hidden rounded-full bg-[rgba(var(--overlay-rgb),0.08)]" role="progressbar" aria-label="Progresso das missões" aria-valuemin={0} aria-valuemax={STUDENT_MISSIONS.length} aria-valuenow={completedCount}>
        ...
        <CardContent id="student-missions-list" className="px-5.5 py-5">
          {!started && completedCount === 0 && (
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-md border border-brand-border bg-brand-dim px-4 py-3">
              <div>
                <p className="font-display text-sm font-bold text-foreground">Bem-vindo à MSY Academy!</p>
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">Vamos conhecer rapidamente as ferramentas que vão ajudar você a organizar seus estudos.</p>
              </div>
              <button type="button" onClick={() => setStarted(true)} className="rounded-sm bg-brand px-3.5 py-2 text-xs font-bold text-primary-foreground transition-colors hover:bg-brand-hover focus-visible:ring-2 focus-visible:ring-ring">
                Começar missões
              </button>
            </div>
          )}
          {pendingMissions.length === 0 && !showAllMissions && (
            <p className="mb-4 rounded-md border border-brand-border bg-brand-dim px-4 py-3 text-sm text-brand-text">
              Você concluiu todas as missões iniciais. Veja todas quando quiser revisar alguma delas.
            </p>
          )}
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <span className={`flex size-9 shrink-0 items-center justify-center rounded-sm ${done ? "bg-brand text-primary-foreground" : "bg-brand-dim text-brand-text"}`} aria-hidden>
                      {done ? <Check className="size-4" strokeWidth={2.5} /> : <Icon className="size-4.5" strokeWidth={1.8} />}
                    </span>
                    <span className={`text-[11px] font-semibold ${done ? "text-brand-text" : "text-muted-foreground"}`}>{done ? "Conhecido" : "Pendente"}</span>
                  </div>
                  <p className="mt-3 font-display text-sm font-bold text-foreground">{mission.title}</p>
                  <p className="mt-1 flex-1 text-xs leading-relaxed text-muted-foreground">{mission.description}</p>
                  <span className="mt-3 text-xs font-bold text-brand-text group-hover:text-brand-hover">{done ? "Acessar novamente" : mission.actionLabel} →</span>
                </Link>
              );
            })}
          </div>
