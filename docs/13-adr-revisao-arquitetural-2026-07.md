...
- **Fase 2:** migração visual página a página (professor → aluno → admin); RHF+Zod por form migrado; Tiptap/Recharts conforme as telas chegam; primitivas de IA extraídas das telas de geração; promover warns de lint a error; deletar CSS legado por seção.
- **Fase 3:** motor de IA — Vercel AI SDK sob o orchestrator; Voyage embeddings + re-ingest; tutor via orchestrator.
- **Fase 4:** Sentry + Vercel Analytics; @upstash/ratelimit; revisão de sanitização (`generated_html`).
