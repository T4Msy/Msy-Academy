 ...
 *
 * One `<Document>` renders one page per sheet, so printing a whole class is
 * a single PDF (one call to renderToBuffer) instead of N separate files.
 */
...
export interface GabaritoSheetData {
...
function GabaritoPage({ qrDataUrl, examTitle, className, studentName, questionCount }: GabaritoSheetData) {
  return (
    <Page size="A4" style={styles.page}>
      ...
      <Text style={styles.instructions}>
        Preencha completamente o círculo correspondente à resposta certa, usando caneta escura. Não amasse, dobre nem
        rasure esta folha — os marcadores nos cantos são necessários para a correção automática.
      </Text>
...
export function GabaritoDocument({ sheets }: { sheets: GabaritoSheetData[] }) {
  return (
    <Document>
      {sheets.map((sheet) => (
        <GabaritoPage key={sheet.answerSheetId} {...sheet} />
      ))}
    </Document>
  );
}
